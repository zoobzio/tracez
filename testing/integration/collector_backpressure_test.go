...
// TestExportLatencyImpact verifies Export() doesn't block collection.
func TestExportLatencyImpact(t *testing.T) {
	tracer := tracez.New("test-service")
	collector := tracez.NewCollector("test", 100)
	tracer.AddCollector("collector", collector)
	defer tracer.Close()

	// Continuous span generation.
	stopGeneration := make(chan bool)
	generated := 0
	...
	go func() {
		for {
			...
			default:
				start := time.Now()
				_, span := tracer.StartSpan(context.Background(), "latency-test")
				span.Finish()
				elapsed := time.Since(start)

				mu.Lock()
				generated++
				generationTimes = append(generationTimes, elapsed)
				mu.Unlock()
			}
		}
	}()

	// Simulate slow export consumer.
	var wg sync.WaitGroup
	for i := 0; i < 3; i++ {
		wg.Add(1)
		go func(id int) {
			...
			for j := 0; j < 5; j++ {
				// Export with simulated processing time.
				spans := collector.Export()

				// Simulate slow processing.
				time.Sleep(100 * time.Millisecond)

				t.Logf("Export %d-%d: %d spans", id, j, len(spans))
			}
		}(i)
	}
	...
	t.Logf("Generated %d spans, max generation time: %v", generated, maxTime)

	// Generation should never block significantly.
	if maxTime > 10*time.Millisecond {
		t.Errorf("Span generation blocked: max time %v", maxTime)
	}
}

...
func testDeepNesting(t *testing.T) {
	tracer := tracez.New("deep-nesting-test")
	defer tracer.Close()
...
func testConcurrentPropagation(t *testing.T) {
	tracer := tracez.New("concurrent-propagation-test")
	defer tracer.Close()
...
func testExtremeDepth(t *testing.T) {
	tracer := tracez.New("extreme-depth-test")
	defer tracer.Close()
...
func testMassiveFanout(t *testing.T) {
	tracer := tracez.New("massive-fanout-test")
	defer tracer.Close()
	
	collector := tracez.NewCollector("test", 20000)
	// Note: collector cleanup handled by tracer.Close() in production use
	collector.SetSyncMode(true)
	tracer.AddCollector("test", collector)
	...
	for _, span := range exported {
		if span.TraceID != rootTraceID {
			t.Errorf("Trace discontinuity in fanout: span %s has wrong trace ID", span.SpanID)
			break
		}
	}
}
